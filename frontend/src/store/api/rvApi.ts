...
export type RVSyntheticDatasetParameters = {
  seed: number
  periods: number
  end_date: string
...
export type RVDatasetMetadata = {
  ...
  end_date: string
  computed_at: string
  synthetic_parameters: RVSyntheticDatasetParameters | null
}
...
export type RVBacktestMetrics = {
  mae: number | null
  rmse: number | null
  correlation: number | null
  change_direction_accuracy: number | null
  n_obs: number
}

export type RVRegimeMetric = {
  regime: RVRegime
...
export type RVFeatureResponse = {
  symbol: string
  estimator: RVEstimatorMetadata
  dataset: RVDatasetMetadata
  points: RVFeatureRow[]
}
export type RVHistoryResponse = {
  symbol: string
  model: RVModelName
  horizon_sessions: number
  estimator: RVEstimatorMetadata
...
): RVHorizonEstimate | undefined => estimates.find((estimate) => estimate.horizon_sessions === horizonSessions)

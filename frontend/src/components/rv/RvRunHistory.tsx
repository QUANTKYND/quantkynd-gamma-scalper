...
const RvRunHistory = ({ runs }: Props): React.ReactElement => {
  if (runs.length === 0) return <Typography color="text.secondary">No backtest runs yet.</Typography>
  return (
    <List disablePadding>
      {runs.map((run, index) => <ListItem key={run.run_id} divider={index < runs.length - 1} disableGutters sx={{ alignItems: 'flex-start', py: 1.5 }}>
        ...
        <ListItemText primary={`${run.model.toUpperCase()} · ${run.horizon_days}d`} secondary={new Date(run.created_at).toLocaleString()} slotProps={{ primary: { variant: 'body2', sx: { fontWeight: 700 } }, secondary: { variant: 'caption' } }} />

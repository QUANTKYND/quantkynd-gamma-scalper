...
The strategy contract defines trading behavior while the simulation-market contract defines the deterministic session clock, synthetic chain, expiries, strike grid, liquidity, multipliers, and futures convention. The active release uses simulator `sim-1.2`, market and run schemas version 2, manifest schema version 2, and path generator version 2. The CLI reports those versions plus the underlying-path and derived executable-market-state hashes alongside every other run identity hash. Runs write immutable full-spec artifacts below `backend/artifacts/simulation/runs`; failures after run identity exists retain a failed manifest. Entry quality, liquidity, theta, and hard delta limits execute deterministically, while expected edge remains deferred to EDGE-1. Position P&L includes entry costs and session P&L includes overnight gaps. The current clock is weekday-only and does not model NSE holidays or special sessions. This offline simulator has no broker order path, live option data, paper routing, or intraday realized variance.

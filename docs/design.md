...
Sequential ingestion uses one repository-aware transition plan in dry-run and commit. After the provider/profile lock, the service validates the explicit catalogue predecessor against the eligible catalogue leaf, compares provider-key continuity by durable economic instrument ID, resolves eligible instrument-version and provider-mapping record IDs, and attaches those records as explicit predecessors. A provider key can continue to identify the same economic instrument across provenance-bound versions, but any historical binding to another economic instrument rejects the catalogue. Disappeared memberships are diff output only and do not close or supersede their instrument or mapping histories.

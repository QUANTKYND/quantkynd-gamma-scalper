...
Final lifecycle and CLI hardening validates subscription transitions against retained per-scope state while preserving global capture order, so independent scopes may interleave without permitting a completed scope to restart. Connection session and source-order scope bindings remain unique across every reconnect in the validated stream. Exact lifecycle duplicates are removed in first-capture order before transition validation and remain reported as provenance. Fixture manifests use explicit schema/type validation, structural frame failures cannot be approved by expected hashes, and normalization results validate their full-provenance and adopted-semantics hashes against immutable result contents.

Provider-identity boundary hardening validates only identities in the payload selected by `FeedResponse.type`: primary quote keys are exact, control-free UTF-8 strings of at most 512 bytes and primary market segments use the same rules with a 128-byte bound. Any malformed selected identity is a whole-frame failure before resolution or normalization, with the decoded response type retained and zero decoded entries. Malformed identities in an ignored coexisting secondary payload remain unadopted structural presence and do not fail the selected primary payload. Subject-resolution failures use one closed four-value vocabulary.

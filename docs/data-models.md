...
`SubscriptionInstrumentSetV1` stores sorted unique non-empty provider keys and derives its count and stable digest internally; callers cannot supply either derived value. Subscription request modes are `ProviderRequestMode | None`, with required request/acknowledgement equality. A changed instrument set begins a new subscription scope. Normalized lifecycle construction canonicalizes enum instances and UTC timestamps even when direct inputs use enum strings or another aware offset.

`FrameNormalizationResultV1.response_type` is optional because malformed or structurally invalid bytes can fail before a response type is available. The adopted status projection includes provider-derived subject identity. The adopted failure projection excludes deferred declaration/presence metadata while retaining controlled adopted failure meaning. Full-result hashing continues to cover all declared structural metadata.

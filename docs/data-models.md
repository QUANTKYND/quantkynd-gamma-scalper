...
Durability-boundary amendment: every raw-frame, quote, status, and raw/normalized lifecycle `source_order` is a strict JSON/Python integer, never a boolean, from zero through `2**63 - 1`. Caller-controlled provider, schema, connection-session, source-order-scope, subscription-scope, source-file, and source-record identifiers occupy at most 512 UTF-8 bytes and are rejected rather than truncated, trimmed, or normalized. Redacted lifecycle reason codes occupy at most 128 UTF-8 bytes in addition to their controlled-vocabulary rules.

Subscription instrument sets reuse the provider-contract-key contract: each key is non-empty, has no leading/trailing whitespace or ASCII controls, and occupies at most 512 UTF-8 bytes. A set contains at most 5,000 unique keys. Request-mode limits are 5,000 for `ltpc`, 3,000 for `option_greeks`, 2,000 for `full_d5`, and 50 for `full_d30`; an unsubscribe event with absent request mode uses the 5,000-key global limit. Lifecycle fixture input is rejected before JSON parsing above 16 MiB, and raw lifecycle batches contain at most 10,000 events.

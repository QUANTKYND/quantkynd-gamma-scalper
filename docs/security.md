...
- Raw, normalized, manifest, and lifecycle inputs fail closed outside signed-BIGINT source order, bounded opaque-ID, subscription-key, request-mode, fixture-byte, and lifecycle-batch limits. UTF-8 limits are measured in bytes; rejected values are never truncated, trimmed, normalized, or logged. Lifecycle fixture size is checked before full JSON parsing.

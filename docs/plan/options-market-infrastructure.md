...
DATA-1.3 status: implementation complete; acceptance evidence recorded; independent review pending. The slice owns bounded deterministic Upstox V3 frame decoding, provider-neutral quote/status normalization, point-in-time catalogue subject resolution, offline connection/subscription lifecycle contracts, synthetic binary fixtures, and canonical offline CLIs. It adds no durable event state and does not alter LIVE-RV.

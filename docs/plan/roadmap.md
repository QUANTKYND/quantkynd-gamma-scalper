...
| DATA-1 point-in-time options model | ACTIVE | DATA-1.0 freezes deterministic semantics. DATA-1.1 supplies the Postgres/Alembic catalogue, instrument, provider-mapping, session, and transactional repository foundation; provider ingestion and market-event persistence remain. |

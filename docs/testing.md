...
Forward, same-effective-time, open historical, and bounded backfill tests resolve predecessors by current knowledge leaf. They prove explicit catalogue, version, and mapping successor edges; one root per scope; stale and missing predecessor rejection; historical visibility only after the backfill is known; and current-market preservation when the bounded successor is ineligible. Dry-run must produce the same transition checks and deterministic semantic diff as commit without writes or artifact retention. Persistence failure tests distinguish accepted idempotency races from semantic collisions, unrelated constraints, and temporal successor conflicts. The restore fixture contains A, forward B, and bounded historical H, all three retained artifact references, both successor edges, and equivalent historical/current reads before and after restore. Acceptance-critical persistence and concurrency tests run with zero skips on the declared PostgreSQL 17 service.

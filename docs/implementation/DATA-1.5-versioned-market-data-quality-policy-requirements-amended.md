...
**Status:** Requirements amended after independent adversarial review. Independent requirements re-review is pending; design and implementation remain unauthorized.
...
The requirements draft and first adversarial review are complete. This document incorporates the accepted resolutions for findings `ADR-001` through `ADR-023`.
...
2. its normalized event `recorded_at <= evaluation_known_as_of`;
3. its owning normalization result `persistence_recorded_at <= evaluation_known_as_of`;
4. its result/event membership belongs to that exact committed normalization result; membership visibility inherits the owning result's atomic persistence boundary;
5. its normalization schema and implementation are supported by the policy version.
...
- `0 < future_offset <= 1,000 ms`: freshness age is clamped to zero only for freshness calculation and no future-timestamp reason fires;
- `future_offset > 1,000 ms`: error `provider_timestamp_in_future` and disposition is ineligible.

The original timestamp remains unchanged for identity, dependency selection, session-date derivation, and audit evidence. A direct caller may therefore obtain an explicit ineligible assessment for a knowledge-visible future-dated target without allowing that target to be selected implicitly by an as-of reconstruction query.
...
### 9.1 Dependency-kind temporal axes
...
| Dependency kind | Market/effective visibility at target time `T` | Knowledge visibility at `K` | Required durable receipt boundary |
|---|---|---|---|
| Normalized target event | loaded explicitly; future offset is evaluated by §8.3 | `available_at <= K` and `recorded_at <= K` | event `recorded_at` |
| Normalization result and membership | exact result owning target | `persistence_recorded_at <= K` | result `persistence_recorded_at`; membership inherits same transaction |
| Provider mapping temporal record | `effective_from <= T < effective_until` or open end | `recorded_at <= K < superseded_at` or open end | temporal-record `recorded_at` |
| Instrument/contract version temporal record | `valid_from <= T < valid_until` or open end | `recorded_at <= K < superseded_at` or open end | temporal-record `recorded_at` |
| Catalogue version temporal record | `effective_from <= T < effective_until` or open end | `recorded_at <= K` | catalogue temporal-record `recorded_at` |
| Trading-session version | session identity derived from policy timezone and exchange date; `open_at/close_at` are eligibility bounds | `recorded_at <= K < superseded_at` or open end | session temporal-record `recorded_at` |
| Segment-status observation | `provider_timestamp <= T` | `available_at <= K`, `recorded_at <= K`, owning result persistence `<= K` | event/result recorded clocks |
| Connection lifecycle observation | `occurred_at <= T` | `available_at <= K` and `recorded_at <= K` | lifecycle `recorded_at` |
| Subscription lifecycle observation | `occurred_at <= T` | `available_at <= K` and `recorded_at <= K` | lifecycle `recorded_at` |
| Subscription instrument set | selected scope's immutable set identity | set row and owning lifecycle assertion recorded `<= K` | set/lifecycle recorded clocks |

A physically later row with a caller-supplied backdated semantic or `recorded_at` value must not rewrite prior truth. For DATA-1.4 market events/results, the existing `persistence_recorded_at` boundary is authoritative. For catalogue, mapping, instrument-version, session, status, and lifecycle dependencies, the pre-implementation design is required to bind each selected record to an append-only repository-owned receipt fact whose timestamp cannot be supplied or backdated by the caller. Migration `20260804_05` may add DATA-1.5 receipt rows and FKs but must not mutate DATA-1.4 semantic records. A dependency is selectable only when both its asserted bitemporal knowledge interval and its receipt fact are visible by `K`. Historical dependency rows without a qualifying receipt fact are not silently selectable.

Reconstructing the same `assessment_id` with a different dependency closure, selected IDs, absence proofs, or canonical evidence is a typed assessment collision/durable-corruption failure and persists nothing.
...
5. `available_at` and `recorded_at` are visibility filters, not state-precedence fields;
...
For a market-segment-status target, its age is `evaluation_market_as_of - status.provider_timestamp`. When status is a dependency of a quote, dependency age is `quote.provider_timestamp - status.provider_timestamp`; a later status is never selectable.

Future timestamp behavior is governed by §8.3. A negative age within the one-second tolerance is clamped to zero only for freshness. A negative age beyond the tolerance fires `provider_timestamp_in_future` and no stale/warning reason.
...
Selected connection and subscription assertions must occur on the same `Asia/Kolkata` exchange date as the target and satisfy:

```text
target.provider_timestamp - assertion.occurred_at <= 43,200,000 ms
```
...
| `provider_timestamp_in_future` | Timestamp exceeds the 1,000 ms tolerance. |
...
9. `market_data_quality_run_assessments`
...
|---|---|---|
| ADR-001 | Separate knowledge visibility from market-time eligibility; retain 1,000 ms tolerance as an evaluated rule. | Time semantics/tests |
...
| ADR-007 | Freeze event `recorded_at`, result `persistence_recorded_at`, and atomic membership visibility. | Knowledge clocks |
| ADR-008 | Add dependency-kind market/knowledge axis table. | Selectors/tests |
| ADR-009 | Require repository-owned non-backdatable receipt boundary; changed closure under same assessment ID is collision/corruption. | Persistence/selectors |
...
## 29. Independent requirements re-review gate

The amended requirements may proceed to design only after an independent reviewer verifies:

- every `ADR-001` through `ADR-023` resolution is reflected in normative wording;
...
- dependency clocks match repository temporal models;
- no implementation, migration, configuration, or test code was introduced during requirements resolution.

A blocker or high-severity re-review finding keeps design and implementation unauthorized.

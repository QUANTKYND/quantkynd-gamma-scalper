...
def whalley_wilmott_band(
    ...
    values = (
        ...
        risk_free_rate,
        maximum_half_width_delta_units,
    )
    if not all(math.isfinite(value) for value in values):
        raise ValueError("Whalley-Wilmott inputs must be finite")
    if transaction_cost_rate < 0 or portfolio_gamma_units_per_inr < 0 or time_to_expiry_years < 0:
        raise ValueError("cost, gamma, and time must be non-negative")
    if risk_aversion_per_inr <= 0 or spot_inr <= 0 or maximum_half_width_delta_units <= 0:
        raise ValueError("risk aversion, spot, and maximum half-width must be positive")
    radicand = (
        3
        * transaction_cost_rate

...
def test_frictionless_hedging_reduces_delta_error_on_controlled_path() -> None:
    ...
    unhedged = summarize(run_simulation(strategy, shocked, "no_hedge", ZERO, ZERO))
    hedged = summarize(run_simulation(strategy, shocked, "fixed_interval", ZERO, ZERO))
...
def test_more_trading_increases_modeled_costs() -> None:
    ...
    unhedged = summarize(run_simulation(strategy, shocked, "no_hedge", ZERO, costs))
    hedged = summarize(run_simulation(strategy, shocked, "fixed_interval", ZERO, costs))
...
def test_loss_limits_exit_deterministically(
    ...
    constrained = strategy.model_copy(update={"risk": risk})
    result = run_simulation(constrained, collapsed, "no_hedge", ZERO, ZERO)
